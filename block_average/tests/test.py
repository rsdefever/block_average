...
def main():
    ...
    for n_sample in n_samples:
        ...
        with open(outfile, 'w') as f:
            f.write('# Average: {:16.4f}, Variance: {:16.4f}\n'.format(average,variance))
            f.write('# N_blk_ops, Mean_est, Var_est, var_err\n')
            for n_blk_ops, (mean_est, var_est, var_err) in enumerate(zip(
                                                                means_est,
                                                                vars_est,
                                                                vars_err)):
                f.write('{:10d}{:18.6f}{:16.4e}{:16.4e}\n'.format(n_blk_ops,mean_est,var_est,var_err))
